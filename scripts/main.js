...
const history = (parent, value) => { //fonction pour constituer un historique des villes consultées durant la session, appelée dans getData
  ...
  Array.from(parent.children).forEach(child => { // on regarde si la ville ne s'y trouve pas déjà /!\ .children renvoie une nodeList qu'il faut convertir en tableau, d'où l'utilisation de Array.from
   ...
   // en parcourant l'ensemble des enfants de select
  ...
  fetch(`https://api.openweathermap.org/data/2.5/forecast?id=${cityId}&units=metric&lang=fr&appid=71364b0781d03bc97f495d5722900a5e`)
    ...
    .then(data => {
      ...
      for (let i = 0; i < data.list.length; i++) {// boucle pour les prévisions
        console.log(dayjs((data.list[i].dt + data.city.timezone) * 1000).format('HH:mm'))
        if (dayjs((data.list[i].dt + data.city.timezone) * 1000).format('HH') === '11' || dayjs((data.list[i].dt + data.city.timezone) * 1000).format('HH') === '12' || dayjs((data.list[i].dt + data.city.timezone) * 1000).format('HH') === '13') { // l'API renvoie une prévision pour toutes les 3 heures sur 5 jours 
          forecast.push({              // On prend en compte l'amplitude horaire de 3h, et on choisit la prévision à la mi-journée en fonction du fuseau horaire          ^       ^-------------------------------------^           ^    
            date: dayjs((data.list[i].dt + data.city.timezone) * 1000).locale('fr').format('ddd D MMM YYYY'),                                                  //Appel librairie    TStamp(unix) du serveur+TZ*conversion    formatage en Heure 
    ...
    .catch((err) => { //on affiche une alerte si il y a eu un problème dans la requête
      alert('Mauvais identifiant, mauvais APPID, ou serveur injoignable ' + err)
    })
}
